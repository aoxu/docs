...
### getUserLog 查询过期的定时任务
查询过的定时任务会立即删除。
####传入参数
{}
####传出参数
返回的data里，取决于callback的类型，params里的参数可能有所区别。
```json
{
    "data":[
        {
            "callback":"通知类型",
            "params":{
                "id":"通知主体的id，比如建筑建造完成就是建筑id",
                "level":"通知主体的等级，升级后的等级是此值+1"
            }
        },
        ...
    ]
...
```json
{
    "count":"整数，可选参数，如果不传默认值为3"
}
```
####传出参数
data字段里是按配置顺序排列的任务数组，每个任务包含配置和用户数据。
```json
{
    "data":[
        {
            ...
            "guide":"bool, 是否引导",
            "status":"整数，1 未完成/2 已完成/3 已领赏",
            "isAnimePlayed":"bool，是否已播放动画"
        },
        ...
    ],
    "completedCount":"已完成或已领赏的任务个数",
    "phaseBonusStatus":"bool, 是否已经领取了阶段奖励"
}
```
